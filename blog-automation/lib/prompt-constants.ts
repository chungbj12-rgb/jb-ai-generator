...
8. 연락처: 글 마지막에 ${JB_CENTER_INFO.phone} 또는 ${JB_CENTER_INFO.mobile} 포함인가? (이 두 번호 외 다른 전화번호가 있으면 실패)
...
- 연락처(글 마지막 필수): ☎ ${JB_CENTER_INFO.phone} / ${JB_CENTER_INFO.mobile}
- 문의 전화번호는 위 두 개만 사용. 다른 번호(다른 010 번호, 지어낸 번호)는 절대 쓰지 않는다
...
- 이 글이 그대로 최종본으로 발행될 수도 있으니, AI가 쓴 티가 나지 않도록 문장 길이·리듬에 변화를 주고
  "~라고 할 수 있습니다", "결론적으로", "이처럼" 같은 상투적인 AI 표현은 피해서 사람이 직접 쓴 것처럼 마무리한다.
